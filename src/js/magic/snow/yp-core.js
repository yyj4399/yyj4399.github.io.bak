...
, o = $({})
  ...
  // 写cookie
  yp.sub('page/cookie/oLoginInfo', function(e, data){
    var t = new Date();
    t.setTime( t.getTime() + 600000 );
    // $.cookie('oLoginInfo', data.cookie_value, { expires: t, path: '/' });
  });
  yp.sub('page/cookie/oViewer', function(e, data){
    var t = new Date();
    t.setTime( t.getTime() + 600000 );
    // $.cookie('oViewer', data.cookie_value, { expires: t, path: '/' });
  });
}(jQuery, yp);
...
, loadCSS = function(url) {
  ...
  api.ready = function() {
    var nReadyCount = 3///
    $.sub('base-ready doc-ready login-ready', function() {
      if (--nReadyCount <= 0) {
        api.ready();
      }
    });
  ...
  api.init = function() {
    ...
    if( -1 != searchStr.indexOf('_r') ){
      $.removeCookie('oViewer');
      $.removeCookie('oLoginInfo');
    }
    var cookie_oLoginInfo = $.cookie('oLoginInfo');
    if( !cookie_oLoginInfo ){
      $.ajax({
        url: oPageConfig.oUrl.sGetLoginInfoUrl
      , dataType: 'json'
      , success: function(data){
          if( 0 == data.code ){
            oPageConfig.oLoginInfo = data.data;
          } else {
            oPageConfig.oLoginInfo = null;
          }
          yp.pub('page/cookie/oLoginInfo', { cookie_value: oPageConfig.oLoginInfo || 1 });

          $.pub('login-ready').cache();
        }
      , error: function(){
          oPageConfig.oLoginInfo = null;
          yp.pub('page/cookie/oLoginInfo', { cookie_value: 1 });

          $.pub('login-ready').cache();
        }
      });
    } else {
      if( 1 == cookie_oLoginInfo ){
        oPageConfig.oLoginInfo = null;
      } else {
        oPageConfig.oLoginInfo = cookie_oLoginInfo;
      }
      $.pub('login-ready').cache();
    }
  };
